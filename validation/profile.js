...
module.exports = function validateProfileInput(data) {
    ...
    data.lindedin = !isEmpty(data.lindedin) ? data.lindedin : "";

    if (!Validator.isLength(data.handle, { min: 2, max: 40 })) errors.handle = "Handle needs to be between 2 and 4 characters";
    if (Validator.isEmpty(data.handle)) errors.handle = "Handle fields is required";
    if (Validator.isEmpty(data.status)) errors.status = "status fields is required";
    if (Validator.isEmpty(data.skills)) errors.skills = "skills fields is required";
    ...
    if (!Validator.isEmpty(data.lindedin)) {
        if (!Validator.isURL(data.lindedin)) errors.lindedin = "Not a valid URL";
    }
